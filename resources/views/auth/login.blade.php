<x-guest-layout class="flex h-screen w-screen items-center justify-center">
    ...
    <form method="POST" action="{{ route('login') }}"
        class="mx-auto mt-10 flex w-full max-w-lg flex-col items-stretch gap-y-6 rounded-lg bg-background p-10">
        @csrf

        <!-- Email Address -->
        <x-ui.input-field-control name="email" placeholder="example@email.com" :error="$errors" :label="'Email'"
            type="email" />

        <!-- Password -->
        <x-ui.input-field-control name="password" placeholder="******" :error="$errors" :label="'Email'"
            type="password" />
