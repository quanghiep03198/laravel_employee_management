...
<x-app-layout>
   ...
   {{-- Form --}}
   <form action="{{ route('api.employee.add') }}" method="POST" class="max-w-7x mx-auto w-full rounded-lg bg-background p-4 xl:gap-x-10 xl:px-6 xl:py-10">
      @csrf
      <div class="mb-6 grid w-full grid-flow-col grid-rows-4 gap-6 sm:grid-flow-row sm:grid-cols-1">
         <x-ui.input-field-control name="user_id" :error="$errors" :label="'User'" placeholder="Select user ..." />
         <x-ui.input-field-control name="first_name" :error="$errors" :label="'First name'" placeholder="Employee's first name ..." />
         <x-ui.input-field-control name="last_name" :error="$errors" :label="'Last name'" placeholder="Employee's last name ..." />
         <x-ui.radio-field-control name="gender" :error="$errors" :label="'Gender'" :options="$genderOptions" />
         <x-ui.input-field-control name="indentity_number" :error="$errors" :label="'Identity number'" placeholder="Employee's ID number ..." />
         ...
         <x-ui.input-field-control name="employee_code" :error="$errors" :label="'Employee code'" placeholder="xxx-xxx-xxx" />
         <x-ui.input-field-control name="hire_date" type="date" :error="$errors" :label="'Hire date'" />
      </div>
      <x-ui.button type="submit" :variant="'primary'" :icon="'floppy'" class="px-4 sm:w-full">{{ __('Save') }}</x-ui.button>
   </form>
</x-app-layout>
