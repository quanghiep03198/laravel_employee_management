<div x-data="{ open: false }" class="sticky top-0 !z-[100]">
		...
		<div class="mx-auto max-w-full px-4 lg:px-8">
			<div class="flex h-16 items-center justify-between xl:justify-end">
				...
				<div class="flex flex-1 items-center divide-x divide-border xl:divide-x-0">
					...
					<div
						class="relative flex flex-1 items-center gap-x-2 bg-background pl-4 xl:p-0">
						<i class="bi bi-search"></i>
						<form action="">
							<input type="text"
								class="block border-none text-sm focus:outline-none focus:!ring-0"
								placeholder="Search ...">
						</form>
					...
					<x-ui.dropdown align="right" width="48">
						<x-slot name="trigger">
							<button
								...
								<img
									class="h-18 w-8 rounded-full ring-offset-2 group-focus:ring-[2px] group-focus:ring-primary"
	...
	<div x-show="open" x-transition:enter="transition ease-in-out duration-300"
		@close.stop="open = false" x-transition:enter-start="-translate-x-full"
	...
	<div x-show="open" @close.stop="open = false"
		x-transition:enter="transition ease-in-out duration-300"
		x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
		x-transition:leave="transition ease-in-out duration-300"
		x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
		class="fixed inset-0 z-40 bg-gray-950/80 xl:hidden" @click="open = false">
	</div>
</div>

<script>
	document.addEventListener('alpine:init', () => {
		Alpine.data('confirmOpen', (w) => ({
			console.log(e)
		}))
	})
</script>
